...
class CifarResNet(nn.Module):
    ...
    def __init__(self, block, depth, zero_init_residual=False, nf=64):
        ...
        self.stage_3, self.pre_stage_3 = self._make_layer(block, 64, layer_blocks, 2, previous = True)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, previous=False):
        downsample = None
    ...
    def previous_feature(self, x, feature=False, T=1, labels=False, scale=None, keep=None):
        x = self.conv_1_3x3(x)
        x = F.relu(self.bn_1(x), inplace=True)
        x = self.stage_1(x)
        x = self.stage_2(x)
        x = self.pre_stage_3(x)
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        return x
