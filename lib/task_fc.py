...
class taskLinear(Module):
    def __init__(self, in_features, out_features, device, bias=True, eta=True, normalize=False):
        super(taskLinear, self).__init__()
    ...
    def forward(self, input):
        ...
        if self.normalize == False:
            if self.bias is not None:
                ...
                out = F.linear(input,concat_weight)
            else:
            ...
            if self.bias is not None:
                ...
                out = F.linear(F.normalize(input, p=2,dim=1), \
                        F.normalize(concat_weight, p=2, dim=1))
            else:
                out = F.linear(F.normalize(input, p=2,dim=1), \
